                    ...
                    <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                        ...
                        <div className="flex flex-col xs:flex-row items-start xs:items-center">
                            ...
                            <div className='xs:ml-auto space-x-4 flex items-center mt-2 xs:mt-0'>
                                <button className="text-white bg-myorange border-0 py-2 px-4 focus:outline-none hover:bg-darkorange rounded" onClick={() => { buyNow(id, name, size.current.value) }}>Buy Now</button>
                                <button className="text-white bg-myorange border-0 py-2 px-4 focus:outline-none hover:bg-darkorange rounded" onClick={() => { editCart('add', id, name, size.current.value) }}>Add to cart</button>
...
export async function getServerSideProps({ query: { id } }) {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}products/get?id=${id}`)
